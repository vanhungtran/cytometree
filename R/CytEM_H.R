...
{
  if(class(M)!="matrix")  {    M <- as.matrix(M)   }
  n <- nrow(M)
  p <- ncol(M)
  
  if(n <= minleaf ) {    return(list("mark_not_dis" = 1:p))   }
  nEMdegenerate <- 5
  
  if(minleaf < nEMdegenerate)   {   minleaf <- nEMdegenerate   }
  aic_norm_old <- -Inf
 # parameters  <- mark_not_dis <- c(first_markers)
  
 # length(first_marker) = p0
 #  M = 
